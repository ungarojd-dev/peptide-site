// Netlify Serverless Function — Ion Peptide Product Feed
// Deployed at: https://mypeptideprice.com/.netlify/functions/ion-products

const CK   = process.env.ION_CK;
const CS   = process.env.ION_CS;
...
async function fetchVariations(productId) {
  ...
  try {
    ...
    console.log(`Ion variation fetch skipped for ${productId}: ${err.message}`);
    return [];
  }
}
...
function mapCategory(product) {
  ...
  if (/glp|semaglutide|tirzepatide|retatrutide|cagrilintide|cagri|mazdutide|orforglipron|survodutide|liraglutide|amycretin|weight|ion-[123][str]|gla-[123]|pep-(?:sm|tz|trz|rt)|tesofensine|metaboflex/.test(t)) return "GLP-1 & Incretin";
