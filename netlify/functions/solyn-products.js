...
const COMPANY       = "Solyn Labs";
const AFFILIATE_URL = "https://partner.solyn.com/?ref=SammyC";
const PER_PAGE      = 100;
  ...
  if (product.type === "variable") {
    ...
    if (variations.length === 0) {
      results.push({ product: product.name, listing: product.name, company: COMPANY, category,
        ...
        image, url: AFFILIATE_URL, source: "api" });
    } else {
      for (const v of variations) {
        ...
        results.push({ product: product.name, listing, company: COMPANY, category,
          ...
          image: v.image?.src || image, url: AFFILIATE_URL, source: "api" });
      }
    }
  } else {
    results.push({ product: product.name, listing: product.name, company: COMPANY, category,
