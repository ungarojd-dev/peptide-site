...
// Bumped to force Netlify to rebundle the functions. vendor-config.json is
// imported here with { type: "json" }, so it is baked in at bundle time: adding
// the Peptidology logo path to that file had no effect while the cached bundle
// still held the old copy with an empty logo, and their rows fell back to
// initials. Any future vendor-config change needs a version bump too.
export const ENGINE_VERSION = "1.5.1-peptidology-logo";

// VENDOR-CONFIG-FINGERPRINT
// Regenerated by scripts/sync-cms-vendors.mjs from data/vendor-config.json.
// Its only job is to change this file whenever that JSON changes, because the
