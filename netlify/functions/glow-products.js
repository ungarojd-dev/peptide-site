...
async function transformProduct(p) {
  ...
  if (p.type === "variable") {
    // Use parent product price range to avoid per-variation API calls
    const minPrice = p.price_html ? p.price_html.replace(/<[^>]+>/g,"").match(/[\d.]+/g) : null;
    const priceDisplay = p.price ? `$${parseFloat(p.price).toFixed(2)}` : "Contact for price";
    results.push({ product: p.name, listing: p.name, company: "Glow Aminos", category, price: priceDisplay, sku: p.sku, in_stock: p.stock_status === "instock", image, source: "api" });
  } else {
