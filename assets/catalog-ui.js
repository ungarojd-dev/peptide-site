(function(global){
  ...
  function bestOffer(card){const offers=allOffers(card);return offers.find(o=>o.supplier.effective_price_min!=null)||offers[0]||null;}
  ...
  function cardHtml(card){
    ...
    const rows=isAll?allOffers(card):(variant.suppliers||[]).map(supplier=>({supplier,variant})).sort((a,b)=>(a.supplier.effective_price_min??Number.POSITIVE_INFINITY)-(b.supplier.effective_price_min??Number.POSITIVE_INFINITY));
    ...
    const totalListings=isAll?rows.length:(variant.suppliers||[]).length;
    const supplierHtml=visible.length?visible.map((row,index)=>supplierRow(row.supplier,card,isAll?row.variant.label:"",index===0&&row.supplier.effective_price_min!=null)).join(""):`<div class="supplier-row supplier-empty-row"><span>No listings available.</span></div>`;
    ...
    return `<article class="product-card ${tone}">
      <header class="product-card-head">
        ...
        <div class="product-title-row">
          ...
          <span class="vendor-count">${esc(card.supplier_count||0)} vendor${card.supplier_count===1?"":"s"}</span>
        </div>
        <div class="card-lowest-block">
          <div class="card-lowest-label">Lowest tracked price</div>
  ...
  function renderCards(scroll){
    ...
    if(status) status.textContent=`Showing ${filtered.length} of ${state.cards.length} product cards`;
  ...
  async function boot(){
    ...
    const fallbackPromise=json("/data/catalog-fallback-snapshot.json?v=premium-index4",7000);
    const latestPromise=json("/.netlify/functions/catalog-snapshot?v=premium-index4",10000);
    applyInitialFilters();
