(function(global){
  ...
  const PAYMENT_GLYPHS={
    ...
    paypal:"wallet",wise:"wallet",affirm:"wallet","idem-pay":"bank",idem:"bank",
  ...
  function supplierRow(supplier,card,variantLabel="",isBest=false){
    ...
    // Analytics keeps a descriptive label even though the button is generic.
    const ctaTracking=supplier.discount_percent>0
      ?`View deal, ${Number(supplier.discount_percent)}% off at ${supplier.vendor_name}`
      :`View deal at ${supplier.vendor_name}`;
  ...
  async function boot(){
    ...
    const fallbackPromise=json("/data/catalog-fallback-snapshot.json?v=20260908-disguised-alpha-labor-v126",7000);
    const latestPromise=json("/.netlify/functions/catalog-snapshot?v=20260908-disguised-alpha-labor-v126",10000);
    applyInitialFilters();
