(function(global){
  ...
  function cardHtml(card){
    const selected=selectedVariantId(card);const isAll=selected===ALL_VARIANTS;const variant=activeVariant(card);
    ...
    const supplierHtml=visible.length?visible.map(row=>supplierRow(row.supplier,card,isAll?row.variant.label:"")).join(""):`<div class="supplier-row" style="padding:12px 0;"><span style="color:var(--muted);font-size:12px;">No listings available.</span></div>`;
    ...
    const lowestPriceHtml=`<div class="card-lowest-block">
      <div class="card-lowest-label">Lowest Price</div>
      <div class="card-lowest-price${lowestPrice?"":" card-lowest-na"}">${esc(lowestPrice||"—")}</div>
      <div class="card-lowest-vendor">${esc(lowestVendor)}</div>
    </div>`;

    const totalListings=isAll?suppliers.length:(variant.suppliers||[]).length;

    // Collapsed: just show the card summary + View Prices button
    if(!isExpanded){
      return `<article class="product-card">
        <div class="product-card-head">
          ...
          <div class="card-meta-row">
            <div class="card-vendor-chip">
              <span class="card-vendor-chip-num">${esc(card.supplier_count)}</span>
              <span>vendor${card.supplier_count===1?"":"s"} available</span>
            </div>
          </div>
        </div>
        <div class="card-action-row">
          <button type="button" class="card-view-btn" data-action="expand" data-card="${attr(card.id)}">
            View Prices <span class="card-view-btn-arrow">→</span>
          </button>
        </div>
      </article>`;
    }

    // Expanded: show full variant + supplier breakdown
    return `<article class="product-card">
      <div class="product-card-head">
        <div class="product-card-meta">${esc(card.category)} · ${esc(card.format)}</div>
        <div class="product-title-row">
      ...
      <div class="card-action-row">
        <button type="button" class="card-view-btn collapse" data-action="expand" data-card="${attr(card.id)}">
          ↑ Collapse
        </button>
      </div>
    </article>`;
